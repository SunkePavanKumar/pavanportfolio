import ayla from '/public/image/ayla.jpg';
import crefin from '/public/image/crefin.jpg';
import realEstate from '/public/image/real-estate.jpg';
import travel from '/public/image/travel.jpg';

export const projectsData = [
    {
        id: 1,
        name: 'FlavorFlow: Redefining Food Ordering',
        description: 'Designed and developed a full-stack food ordering application using Node.js, Express.js, MongoDB, React, Redux, Stripe, OAuth, and other technologies. The application allows users to browse menus, customize orders, create secure accounts, and make payments through Stripe. It also utilizes RabbitMQ for efficient order processing.',
        tools: ['Node.js', 'Express.js', 'MongoDB', 'React', 'Redux', 'Stripe', 'OAuth', 'bcrypt', 'passport.js', 'JSON Web Tokens', 'JavaScript', 'Tailwind CSS', 'Modular CSS', 'Vercel', 'Render'],
        role: 'Full Stack Developer',
        code: '',
        demo: 'https://ecommerce-lovat-three.vercel.app/',
        image: 'FlavorFlow'
    },
    {
        id: 2,
        name: 'FullStack Twitter Clone',
        description: 'Designed and launched a Twitter-like social media platform using cutting-edge technologies. The application features real-time updates with GraphQL and Redis caching, secure user authentication with Google OAuth, and efficient database interactions using Prisma ORM with PostgreSQL. The platform enhances user engagement through a responsive and scalable architecture.',
        tools: ['Node.js', 'Express.js', 'GraphQL', 'Prisma ORM', 'PostgreSQL', 'Supabase', 'Redis', 'Google OAuth', 'JSON Web Tokens', 'Next.js', 'Tailwind CSS', 'React Query', 'GraphQL Request', 'TypeScript', 'AWS'],
        role: 'Full Stack Developer',
        code: '',
        demo: '',
        image: 'FullStackTwitterClone'
    }
    
];


// Do not remove any property.
// Leave it blank instead as shown below

// {
//     id: 1,
//     name: '',
//     description: "",
//     tools: [],
//     role: '',
//     code: '',
//     demo: '',
//     image: crefin,
// },